...
class Chat(object):
    ...
    def get_location(self, contact=None, timestamp_tick=None):
        """return location by timestamp for the given contact in the given timespan.
        ...
        :returns: list of :class:`deltachat.chat.Location` objects.
        """
        if timestamp_tick is None:
            time_tick = 0
        else:
            time_tick = calendar.timegm(timestamp_tick.utctimetuple())
        ...
        dc_array = lib.dc_get_location(self.account._dc_context, self.id, contact_id, time_tick)
        return [
            Location(
                latitude=lib.dc_array_get_latitude(dc_array, i),
                longitude=lib.dc_array_get_longitude(dc_array, i),
