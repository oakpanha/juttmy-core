...
const CORE_FOLDER: &str = "MyJuttmy";
    ...
    pub async fn configure_folders(&mut self, context: &Context, create_mvbox: bool) -> Result<()> {
        ...
        if let Some(ref mut session) = &mut self.session {
            ...
            if mvbox_folder.is_none() && create_mvbox {
                info!(context, "Creating MVBOX-folder \"{}\"...", CORE_FOLDER);

                match session.create(CORE_FOLDER).await {
                    Ok(_) => {
                        //bk: mvbox_folder = Some("Juttmy".into());
                        mvbox_folder = Some(CORE_FOLDER.into());
...
fn get_fallback_folder(delimiter: &str) -> String {
    format!("INBOX{}{}", delimiter, CORE_FOLDER)
}
