...
#[no_mangle]
pub unsafe extern "C" fn dc_get_location(
    context: *mut dc_context_t,
    chat_id: u32,
    contact_id: u32,
    timestamp_tick: i64
    ...
    if context.is_null() {
        eprintln!("ignoring careless call to dc_get_location()");
        return ptr::null_mut();
    }
    let ctx = &*context;

    block_on(async move {
        let res = location::get_location_by_timestamp(
            &ctx,
            ChatId::new(chat_id),
            contact_id,
            timestamp_tick as i64
        )
        .await;
        Box::into_raw(Box::new(dc_array_t::from(res)))
    })
