 ...
 *                    changes require restarting IO by calling dc_stop_io() and then dc_start_io().
 * - `mvbox_watch`  = 1=watch `Juttmy`-folder for changes (default),
 *                    0=do not watch the `Juttmy`-folder,
 *                    changes require restarting IO by calling dc_stop_io() and then dc_start_io().
 * - `mvbox_move`   = 1=heuristically detect chat-messages
 *                    and move them to the `Juttmy`-folder,
 ...
 *   The name of the backup is typically `juttmy-<day>.tar`, if more than one backup is create on a day,
 *   the format is `juttmy-<day>-<number>.tar`
 *
 ...
 * Juttmy extra
*/
dc_array_t* dc_get_location                (dc_context_t* context, uint32_t chat_id, uint32_t contact_id, int64_t timestamp_tick);
